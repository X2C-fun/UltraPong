...
- 14 Rust tests cover deterministic simulation, bounds, maximum-speed defenses on every player edge for arena sizes 2–8, collapsing shapes, speed increases, stage ball reset, pickup effects and expiry, split limits, periodic obstacles, persistent hazards, synchronized countdown and legacy-state compatibility.
- Twelve browser-logic regression tests cover both directions for all 35 player/arena combinations, paddle boundaries, mouse/touch input, smooth paddle motion, short-code parsing, confirmed ball interpolation, safe warmup interpolation and competing finalization.
